 ...
 * @constructor
 */
function GameStyleText(style, initialValue, padding) {
var value;
var color;
var peakColor;
// v4
...
var zero_padding;
	...
	(function ctor(style, initialValue, padding) {
...
//		surface.setFillTexture(gVars.textures.scorebacking);
		...
		surface.setFontColor(sgxColorRGBA.white);
		...
		function processScore(txt, x, y, bDraw, scale) {
			var font = sgx.graphics.FontManager.get().getFont('ui');
			var txt = "000000" + txt;
			    txt = sgxSubstring(txt, txt.length - zero_padding, zero_padding);
			var w = 0;
			var cw;
			var ch;
			var character;
		...
		var w = processScore(value, x, y, false, pulseSize/16);
		processScore(value, x-w/2, y, true, pulseSize/16);
